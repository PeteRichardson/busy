...
mod tests {
    ...
    fn align_resolves_from_the_flag_then_the_file_then_nothing() {
        ...
        // The API defines no default, and the device's implicit anchor is
        // top_left (measured). We deliberately override that with Center so
        // the zero-argument case is centred on the display.
...
This one is unaffected by the centred defaults because it sets `-x`, `-y`, and `--align` explicitly. The **minimal** command, `busy --dry-run text "Hello, World!"`, must instead show the defaults:
...
The `x: 36, y: 8, align: "center"` trio is the centre of the 72×16 front display. Measured on real hardware: this centres `Hi` at x 30–40, `Build Failed!` at x 2–69, and `Hello, World!` at x 1–70, with descenders clearing the bottom edge. Drawing to `--screen back` centres on (80, 40) instead.
...
The front display is 72×16 and the back is 160×80. Three distinct things can make a request the device accepts render nothing, or the wrong thing, while still returning `200 OK`. All three were measured on real hardware (API 25.0.0) by drawing known payloads and reading frames back through `GET /screen`.

1. **The anchor point is off the display.** The obvious case.
2. **The anchor is in bounds but the element extends off-screen.** `align` selects *which point of the element* sits at `(x, y)`, so the direction it extends from there matters. Measured: at `(0,0)`, five of the nine align values — `top_right`, `mid_right`, `bottom_left`, `bottom_mid`, `bottom_right` — produce a frame byte-identical to a cleared screen. An anchor-point check cannot see this, because `(0,0)` is in bounds.
3. **The text is wider than the display.** The device clips at the edge and reports nothing. Measured: `Hello, World!` in `large` inks 70px and fits inside 72px, while `Deployment completed OK` runs past the right edge and loses its tail. Centred, it loses both ends.

These are warnings, never errors: the user may know something the CLI does not.
...
**Interfaces:**
- Consumes: `device::{Align, DisplayElements, ElementKind, Font, Screen}`.
...
pub fn bounds_warnings(payload: &DisplayElements) -> Vec<String> {
    ...
    for element in &payload.elements {
        ...
        if let ElementKind::Text(text) = &element.kind {
            let estimated = (text.text.as_str().chars().count() as f32
                * px_per_char(text.font))
            .round() as i16;
